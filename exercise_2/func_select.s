...
run_func: # void run_func(int choice, Pstring *pstr1, Pstring *pstr2);
    ...
    #movq   %rsi, -16(%rbp)        # save pointer to pstr1 (unnecessary here)
    movq   %rdx, -8(%rbp)         # save pointer to pstr2
    # Alternative way to do it but I am not sure it is that good because it created segfaults at times. TODO: check this later if you have time
    #pushq   %rsi                  # save pointer to pstr1
    #pushq   %rdx                  # save pointer to pstr2

    movq    %rsi, %rdi  # set pstr1 as the first argument
    xorq    %al, %al  # clear %al before function call (customary)
    call    pstrlen     # pstrlen(&pstr1);

    movq    %rax, -16(%rbp)         # save the returned length from pstrlen(&pstr1) in the stack
    movq    -8(%rbp), %rdi          # restore the pointer to pstr2 to %rdi (we will use this as a first argument)
    xorq    %al, %al                # clear %al before function call (customary)
    call    pstrlen                 # pstrlen(&pstr1);
    ...
    movq    -16(%rbp), %rsi         # retore the returned length from pstrlen(&pstr1) from the stack
    ...
    #movq   %rsi, -16(%rbp)        # save pointer to pstr1 (unecessary here)
    movq   %rdx, -8(%rbp)         # save pointer to pstr2
    ...
    movq    -8(%rbp), %rdi              # restore the pointer to pstr2 to %rdi (we will use this as a first argument)
    xorq    %rax, %rax                  # clear %al before function call (customary)
    call    swapCase                    # swapCase(&pstr1);
    movq    $choise_33_34_37_txt, %rdi  # load the text to first argument
    ...
    # TODO: Think how to handle input of i and j that are bigger than 254
    # valid range for i and j: [1, 254]
    movzb  -16(%rbp), %dl    # save the value of i as a third argument (should this be an 8 bit register or bigger?)
    movzb  -15(%rbp), %cl    # save the value of j as a fourth argument
    ...
    movq    $choise_33_34_37_txt, %rdi  # load the text to first argument
    movq    -8(%rbp), %r10              # move len of pstr2 to temporary caller saved register r10
    ...
    end_run_func:
        ...
        # boiler plater code to exit function
        movq    %rbp, %rsp
        popq    %rbp
        ret
